...
def advanced_menu():
    ...
    Main.advanced_menu_window = menu
    ...
    for i in range(Main.max_loops):
        ...
        instrument_lock_check = tk.Checkbutton(
            ...
            command=lambda idx=i: Looping.toggle_instrument_lock(idx)
        )
        ...
        key_lock_check = tk.Checkbutton(
            ...
            command=lambda idx=i: Looping.toggle_key_lock(idx)
        )
        ...
        octave_lock_check = tk.Checkbutton(
            ...
            command=lambda idx=i: Looping.toggle_octave_lock(idx)
        )
        ...
        stop_loop_button = tk.Button(
            slot_frame,
            text="Stop",
            command=lambda idx=i: Looping.stop_loop_by_slot(idx)
        )
        stop_loop_button.pack(side='right', padx=padding_x/2)

        # Store frame, label, and variables
        Main.looping_slot_frames.append({
            'frame': slot_frame,
            'label': slot_label,
    ...
    lock_all_octaves_button = tk.Button(
        ...
        command=Looping.lock_all_octaves
    )
    ...
    unlock_all_octaves_button = tk.Button(
        ...
        command=Looping.unlock_all_octaves
    )
    ...
    if Main.running:
        menu.bind("<KeyPress>", Looping.key_press)
        menu.bind("<KeyRelease>", Looping.key_release)
...
def update_looping_notes_display(event=None):
    ...
    for i, slot_info in enumerate(Main.looping_slot_frames):
        ...
        if note_id is not None:
            ...
            used_instrument = note_info['locked_instrument'] if note_info['instrument_locked'] else Main.current_folder
...
def start_harp():
    ...
    root.bind("<KeyPress>", Looping.key_press)
    root.bind("<KeyRelease>", Looping.key_release)
